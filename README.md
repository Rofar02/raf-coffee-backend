...
- **Сайт (nginx + HTML):** http://localhost:8080 — главная; **меню:** http://localhost:8080/menu; **админка (добавить блюдо):** http://localhost:8080/admin (токен = `ADMIN_TOKEN` из compose). Старые URL `*.html` редиректят на «чистые». JSON меню: `GET /api/menu/`; админ-API: `/admin/...`; `/static/...` — как раньше.  
